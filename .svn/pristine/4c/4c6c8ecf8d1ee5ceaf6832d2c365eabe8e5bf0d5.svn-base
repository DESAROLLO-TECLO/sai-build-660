...
angular.module('siidfApp').directive('rowsPerPageGar', function ($rootScope) {
    return {
        restrict: 'E',
        ...
        controller: function ($scope, $rootScope) { 
            ...
            $scope.rowsx = function(){
        		...
        		for(var i=0;i<$scope.customarray.length ; i++){
        			if($scope.customarray[i].legend=='All' || $scope.customarray[i].legend=='Todos'){
        				var todos = i;
        				i = $scope.customarray.length;
        			}
        		}
        		$scope.rowsPerPage = $scope.customarray[todos].rows;
