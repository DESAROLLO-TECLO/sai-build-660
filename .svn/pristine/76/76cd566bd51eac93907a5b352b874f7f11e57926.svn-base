angular.module("siidfApp").controller("catBitacoraController", function($scope, $filter, ModalService, bitacoraService) {
	...
	ListaComponentesBitacora=function(){
		bitacoraService.obtenerComponentes().success(function(data){
			        inicializaScripts();
...
$scope.changeObtenerListaConceptosBitacora=function(idComponente){
		if(idComponente != null && idComponente != undefined){
			...
			bitacoraService.obtenerConceptos(idComponente).success(
					function(data){
	...
	if ($scope.form.$invalid) {			
       ...
       // showAviso('Formulario incompleto');
		...
		var objectParametros={
				...
				conceptoNombre:articulos 
		};
		
		//copyParametrosBusqueda(objectParametros);
		$scope.listaBitacoraCambios=[];
		$scope.mensaje="";
		bitacoraService.consultaBitacoraCambios(objectParametros).success(
				function(data){
					var listaVO=data;
...
$scope.descargaExcel = function(){
	bitacoraService.descargarExcel("CambiosBitacoraNuevos.xls").success(function(data, status, headers) {			
