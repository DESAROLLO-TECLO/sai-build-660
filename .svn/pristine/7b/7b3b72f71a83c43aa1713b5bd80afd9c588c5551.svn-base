angular.module("siidfApp").controller("infraccionArticuloTotalController", function($scope, ModalService,ReporteDinamicosServices,infraccionArticuloTotalService) {
	...
	CatalogoArticulos = function (){
	  ...
	  $scope.descargaExcel = function(){
		  infraccionArticuloTotalService.descargarExcel("TotalInfraccionesporArticulo.xls").success(function(data, status, headers) {			
	  ...
	  $scope.consultaInfraccionesArticulo=function(parametrosVO){
	  		if ($scope.form.$invalid) {
	  			...
	  			$scope.ResultadoConsulta = [];
	  			infraccionArticuloTotalService.consultaInfraccionesArticulo(objectParametros).success(function(data){
	  				$scope.mensaje ="";
	  				 if(data.length===5000){
