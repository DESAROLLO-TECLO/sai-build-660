angular.module('siidfApp').factory('msjModalFactory',[function($translate){
...
//	Administración
	...
	listaMensajes.push({mensajeEs: '¿Está seguro de guardar esta configuración?',  mensajeEn: 'Are you sure you save this configuration?'});///REPORTES DINAMICOS,ASIGNACION DE REPORTE
	listaMensajes.push({mensajeEs: 'Configuración guardada correctamente',  mensajeEn: 'Configuration saved correctly'});
	listaMensajes.push({mensajeEs:'Algo salió mal al procesar la solicitud',mensajeEn:'Something went wrong when processing the request'});
	listaMensajes.push({mensajeEs:'Los cambios se perderán, ¿Desea continuar?',mensajeEn:'The changes will be lost, do you want to continue?'});
	//listaMensajes.push({mensajeEs:'',mensajeEn:''});
