angular.module("siidfApp").controller("infraccionesTotalAnualesController", function($scope, ModalService,infraccionesAnualesService,reporteService) {
	$scope.anioBuscar="";
   ...
   $scope.consultaInfraccionesTotalAnuales = function (anioBuscar){
	   if ($scope.form.$invalid) {
           ...
           //showAviso('Formulario incompleto');
			...
			infraccionesAnualesService.consultaInfraccionesTotalAnual(anioBuscar).success(function(data){
				$scope.mensaje ="";
				if(data.length>5000){
   ...
   $scope.descargaExcel = function(){///////////////////////////////////////////////
	   infraccionesAnualesService.descargarExcel("InfraccionesAnuales(Total Anual).xls").success(function(data, status, headers) {			
