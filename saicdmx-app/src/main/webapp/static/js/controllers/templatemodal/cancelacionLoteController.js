angular.module('siidfApp').controller('cancelacionLoteController', function($scope, $element, $interval, value, fotoMultaService, radarArchivoProcesadosService) {
	...
	$scope.cancelacionArchivoComplementado = function(){
		if($scope.form.$invalid){
			...
			fotoMultaService.cancelarArchivo($scope.params.lote, $scope.params.motivo).success(function(data){
	...
	buscarArchivoaCancelar = function(){
		fotoMultaService.buscarArchivoaCancelar($scope.params.lote).success(function(data){
			$scope.resumenVO = data;
			if(!data.requiredUpdate){
